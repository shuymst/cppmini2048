...
void Board::init() {
    ...
    right_action_inv_.resize(0x1000);
    ...
    for (a[0] = 0; a[0] < 11; ++a[0]) {
        for (a[1] = 0; a[1] < 11; ++a[1]) {
            for (a[2] = 0; a[2] < 11; ++a[2]) {
                ...
                right_action_inv_[tmp_].push_back(tmp); // tmp_を生成する集合にtmpを追加
            }
        }
    }
}
...
void Board::spawn_force(Square sq, int num) {
    tiles[sq] = num;
}
...
std::vector<uint64_t> Board::inverse_action(Action action) const {
    std::vector<uint64_t> possible_prev_state_keys;
    if (action == RIGHT_ACTION) {
        uint16_t row0 = tiles[SQ_1A] | (tiles[SQ_1B] << 4) | (tiles[SQ_1C] << 8);
        uint16_t row1 = tiles[SQ_2A] | (tiles[SQ_2B] << 4) | (tiles[SQ_2C] << 8);
        uint16_t row2 = tiles[SQ_3A] | (tiles[SQ_3B] << 4) | (tiles[SQ_3C] << 8);

        std::vector<uint16_t> possible_row0 = right_action_inv_[row0];
        std::vector<uint16_t> possible_row1 = right_action_inv_[row1];
        std::vector<uint16_t> possible_row2 = right_action_inv_[row2];

        for (uint16_t r0 : possible_row0) {
            for (uint16_t r1 : possible_row1) {
                for (uint16_t r2 : possible_row2) {
                    uint64_t k = 0;
                    k |= (uint64_t)r0;
                    k |= ((uint64_t)r1 << 12);
                    k |= ((uint64_t)r2 << 24);
                    if (k != key()) {
                        // 自分の同じ盤面は追加しない
                        possible_prev_state_keys.push_back(k);
                    }
                }
            }
        }
        ...
        std::vector<uint16_t> possible_line0 = right_action_inv_[line0];
        std::vector<uint16_t> possible_line1 = right_action_inv_[line1];
        std::vector<uint16_t> possible_line2 = right_action_inv_[line2];

        for (uint16_t l0 : possible_line0) {
            for (uint16_t l1 : possible_line1) {
                for (uint16_t l2 : possible_line2) {
                    uint64_t k = 0;
                    k |= ((uint64_t)l0 & 0xF);
                    k |= (((uint64_t)l1 & 0xF) << 4);
                    k |= (((uint64_t)l2 & 0xF) << 8);
                    k |= ((((uint64_t)l0 >> 4) & 0xF) << 12);
                    k |= ((((uint64_t)l1 >> 4) & 0xF) << 16);
                    k |= ((((uint64_t)l2 >> 4) & 0xF) << 20);
                    k |= ((((uint64_t)l0 >> 8) & 0xF) << 24);
                    k |= ((((uint64_t)l1 >> 8) & 0xF) << 28);
                    k |= ((((uint64_t)l2 >> 8) & 0xF) << 32);
                    if (k != key()) {
                        // 自分の同じ盤面は追加しない
                        possible_prev_state_keys.push_back(k);
                    }
        ...
        for (uint16_t r0 : possible_row0) {
            for (uint16_t r1 : possible_row1) {
                for (uint16_t r2 : possible_row2) {
                    uint64_t k = 0;
                    k |= (((uint64_t)r0 >> 8) & 0xF);
                    k |= ((((uint64_t)r0 >> 4) & 0xF) << 4);
                    k |= (((uint64_t)r0 & 0xF) << 8);
                    k |= ((((uint64_t)r1 >> 8) & 0xF) << 12);
                    k |= ((((uint64_t)r1 >> 4) & 0xF) << 16);
                    k |= (((uint64_t)r1 & 0xF) << 20);
                    k |= ((((uint64_t)r2 >> 8) & 0xF) << 24);
                    k |= ((((uint64_t)r2 >> 4) & 0xF) << 28);
                    k |= (((uint64_t)r2 & 0xF) << 32);
                    if (k != key()) {
                        // 自分の同じ盤面は追加しない
                        possible_prev_state_keys.push_back(k);
                    }
        ...
        for (uint16_t l0 : possible_line0) {
            for (uint16_t l1 : possible_line1) {
                for (uint16_t l2 : possible_line2) {
                    uint64_t k = 0;
                    k |= (((uint64_t)l0 >> 8) & 0xF);
                    k |= ((((uint64_t)l1 >> 8) & 0xF) << 4);
                    k |= ((((uint64_t)l2 >> 8) & 0xF) << 8);
                    k |= ((((uint64_t)l0 >> 4) & 0xF) << 12);
                    k |= ((((uint64_t)l1 >> 4) & 0xF) << 16);
                    k |= ((((uint64_t)l2 >> 4) & 0xF) << 20);
                    k |= (((uint64_t)l0 & 0xF) << 24);
                    k |= (((uint64_t)l1 & 0xF) << 28);
                    k |= (((uint64_t)l2 & 0xF) << 32);
                    if (k != key()) {
                        // 自分の同じ盤面は追加しない
                        possible_prev_state_keys.push_back(k);
                    }
    ...
    return possible_prev_state_keys;
}

std::vector<Action> Board::invertible_actions() const {
    std::vector<Action> res;
    if (is_invertible_by(RIGHT_ACTION)) {
        res.push_back(RIGHT_ACTION);
    }
    if (is_invertible_by(DOWN_ACTION)) {
        res.push_back(DOWN_ACTION);
    }
    if (is_invertible_by(LEFT_ACTION)) {
        res.push_back(LEFT_ACTION);
    }
    if (is_invertible_by(UP_ACTION)) {
        res.push_back(UP_ACTION);
    }
    return res;
}

bool Board::is_invertible_by(Action action) const {
    if (action == RIGHT_ACTION) {
        uint16_t row0 = tiles[SQ_1A] | (tiles[SQ_1B] << 4) | (tiles[SQ_1C] << 8);
        uint16_t row1 = tiles[SQ_2A] | (tiles[SQ_2B] << 4) | (tiles[SQ_2C] << 8);
        uint16_t row2 = tiles[SQ_3A] | (tiles[SQ_3B] << 4) | (tiles[SQ_3C] << 8);

        if (right_action_inv_[row0].size() > 0 && right_action_inv_[row1].size() > 0 && right_action_inv_[row2].size() > 0) return true;
        ...
        if (right_action_inv_[line0].size() > 0 && right_action_inv_[line1].size() > 0 && right_action_inv_[line2].size() > 0) return true;
...
std::vector<uint64_t> Board::enum_before_spawn_normalized_keys() const {
    std::vector<uint64_t> normalized_prev_state_keys;

    for (Square sq = SQ_1A; sq < SQUARE_NB; ++sq) {
        if (get_num(sq) == 1 || get_num(sq) == 2) {
            Board state_board(0);
            state_board.set(key());
            state_board.spawn_force(sq, 0);
            uint64_t normalized_state_key = state_board.normalize();

            if (std::find(normalized_prev_state_keys.begin(), normalized_prev_state_keys.end(), normalized_state_key) == normalized_prev_state_keys.end()) {
                normalized_prev_state_keys.push_back(normalized_state_key);
            }
        }
    }

    return normalized_prev_state_keys;
}
...
std::vector<uint64_t> Board::enum_before_action_normalized_keys() const {
    ...
    for (Action action : invertible_actions()) {
        ...
        for (uint64_t afterstate_key : prev_afterstate_keys) {
            ...
            if (std::find(normalized_prev_afterstate_keys.begin(), normalized_prev_afterstate_keys.end(), normalized_afterstate_key) == normalized_prev_afterstate_keys.end()) {
                normalized_prev_afterstate_keys.push_back(normalized_afterstate_key);
            }
        }
    }

    return normalized_prev_afterstate_keys;
}
